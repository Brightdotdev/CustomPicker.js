...
["FFFFFF", "White"]
  ...
  // Initialize the names array by adding RGB and HSL values to each color
  init() {
    let color, rgb, hsl;
  ...
  // Function to find the closest matching color name for a given hex color
  name(color) {
  ...
  colorName(color) {
    ...
    // Add '#' if color length is a multiple of 3
    if (color.length % 3 === 0) {
      color = "#" + color;
    }
    ...
    for (let i = 0; i < this.names.length; i++) {
      ...
      // Calculate the difference (distance) for closest match
      ndf1 = Math.pow(r - this.names[i][2], 2) + Math.pow(g - this.names[i][3], 2) + Math.pow(b - this.names[i][4], 2);
      ndf2 = Math.pow(h - this.names[i][5], 2) + Math.pow(s - this.names[i][6], 2) + Math.pow(l - this.names[i][7], 2);
      ndf = ndf1 + ndf2 * 2;
    ...
    // Return closest match name or invalid color
    return (cl < 0 ? "Invalid Color: " + color : this.names[cl][1]);
}
