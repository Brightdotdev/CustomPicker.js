...
// Define the NTC class to encapsulate all the functions

export class NTC {
  private names: NamedColor[] = [];
...
["FFFFFF", "White"]
 ...
 /**
   * Initialize color names array by calculating RGB/HSL for each color.
   */
  private init(rawNames: [string, string][]): void {
    this.names = rawNames.map(([hex, name]) => {
  ...
  /**
   * Get the closest matching color name for a given HEX code.
   */
  public getColorName(hex: string): string {
    const normalizedHex = this.normalizeHex(hex);
    if (!normalizedHex) return `Invalid Color: ${hex}`;

    const rgb = this.hexToRgb(normalizedHex);
    const hsl = this.hexToHsl(normalizedHex);

    let closest: NamedColor | null = null;
    let closestDistance = Infinity;

    for (const color of this.names) {
      // Exact match shortcut
      if (normalizedHex === color.hex) return color.name;

      // Distance calculation (RGB + HSL weighting)
      const rgbDistance =
    ...
    return closest ? closest.name : `Invalid Color: ${hex}`;
  }

  /**
   * Normalize a hex color string (#RRGGBB or #RGB) into full #RRGGBB form.
   */
  private normalizeHex(hex: string): string | null {
    hex = hex.toUpperCase();
    if (!/^#?[0-9A-F]{3,6}$/.test(hex)) return null;
